...
def build_manager_data(user: sqlite3.Row, default_avatar: str, start_date: str, 
    ...
    # Прибыль = сумма платежей по сделкам менеджера за период за вычетом себестоимости 
    profit = 0 
    ...
    metrics = [
        ...
        supply_requests_metric if not is_intern else None,
...
def kps():
    ...
    for file in files:
        ...
        if deal['pipeline_id'] == 21:  # Исп. договора
            ...
            if deal['fail_reason']:
                result = f"{deal['fail_reason']}"
                if deal['fail_reason'] not in ('N/A', 'не найдено'):
                    result = f'<b>{result}</b>'
            else:
